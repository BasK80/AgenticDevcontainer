...
Squid image: `squid.conf` (ACL + the firewall-aware `deny_info` error page), `features/` (the always-on `_baseline.list` plus one `.list` per toggleable feature-set), `build-acl.sh` (merges baseline + enabled features + manual + TTL into the live ACL), `ERR_FIREWALL_BLOCKED` (the plain-text page served on a blocked request), `entrypoint.sh`, `watcher.sh` (hot-reloads policy every 5s), `blockfeed.sh` (read-only HTTP feed of recent blocks on `:8099`), `fw` (management script — see [Manage the allowlist](allowlist.md#manage-the-allowlist-from-the-host)).
